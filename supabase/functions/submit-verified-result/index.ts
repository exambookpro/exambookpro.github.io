...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  const {data:result,error:ie}=await admin.from("test_results").insert({user_id:user.id,test_id:test.id,test_title:test.title,score:correct,total_questions:total,attempted,correct,wrong,skipped,accuracy,is_verified:true,time_used_seconds:timeUsed}).select("id,score,total_questions,attempted,correct,wrong,skipped,accuracy,time_used_seconds,created_at").single();if(ie)throw new Error("Could not save result");
